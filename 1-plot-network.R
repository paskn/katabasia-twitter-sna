...
ad_gg_sub <- subgraph.edges(gg, V(network)[name %in% high_degree_nodes])
ad_sub <- get.adjacency(ad_gg_sub,sparse=FALSE)
CairoPNG("./plots/katab_ego_comm_sub.png", width = 2000, height = 1800)
par(mar=c(.01,.01,.01,.01))
gplot(ad_sub, mode="kamadakawai", gmode = "graph",
      vertex.col = V(ad_gg_sub)$color,
